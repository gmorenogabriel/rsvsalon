...
$config = require('../conexion/rutaLog.php');
...
error_log($config['ahora'] . ' - comprobarcedula.php vengo desde: ' . $_SERVER['HTTP_REFERER'] . "\n", 3, $config['ruta']); 
...
if($row){
	...
	error_log($config['ahora'] . ' - ' . $_SESSION['paciente'] . " - " . $_SESSION['cedula'] . "\n", 3, $config['ruta']);
